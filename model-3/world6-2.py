# 6 neorons but 2 layered( not working)
...
# Global for UI communication (3 Inputs, 6 Hidden1, 6 Hidden2, 9 Outputs)
ui_data = {"inputs": [0.0]*3, "hidden1": [0.0]*6, "hidden2": [0.0]*6, "outputs": [0.0]*9}

# ================= 1. THE BRAIN =================
class DeepRobotBrain:
    def __init__(self):
        # 3 Inputs -> 6 Hidden1 -> 6 Hidden2 -> 9 Outputs (8 wheels + 1 STOP)
        # Hidden Layer 1: (3 inputs + 1 bias) * 6 nodes = 24 knobs
        self.hidden1_knobs = [random.uniform(-1.0, 1.0) for _ in range(24)] 
        # Hidden Layer 2: (6 nodes + 1 bias) * 6 nodes = 42 knobs
        self.hidden2_knobs = [random.uniform(-1.0, 1.0) for _ in range(42)] 
        # Output Layer: (6 nodes + 1 bias) * 9 output nodes = 63 knobs
        self.action_knobs = [random.uniform(-1.0, 1.0) for _ in range(63)] 

    def make_tweaked_clone(self, strength):
        baby = DeepRobotBrain()
        for i in range(len(self.hidden1_knobs)):
            baby.hidden1_knobs[i] = self.hidden1_knobs[i] + random.uniform(-strength, strength)
        for i in range(len(self.hidden2_knobs)):
            baby.hidden2_knobs[i] = self.hidden2_knobs[i] + random.uniform(-strength, strength)
    ...
    def decide_action(self, angle, distance, yaw):
        ...
        # Calculate Layer 1 (6 Hidden Thoughts)
        hidden1_thoughts = [0.0] * 6
        knob_idx = 0
        for h in range(6):
            score = (inputs[0] * self.hidden1_knobs[knob_idx] + 
                     inputs[1] * self.hidden1_knobs[knob_idx+1] + 
                     inputs[2] * self.hidden1_knobs[knob_idx+2] + 
                     self.hidden1_knobs[knob_idx+3]) # Bias
            knob_idx += 4
            hidden1_thoughts[h] = math.tanh(score)

        # Calculate Layer 2 (6 Deeper Thoughts) based on Layer 1
        hidden2_thoughts = [0.0] * 6
        knob_idx = 0
        for h2 in range(6):
            score = 0
            for h1 in range(6):
                score += hidden1_thoughts[h1] * self.hidden2_knobs[knob_idx]
                knob_idx += 1
            score += self.hidden2_knobs[knob_idx] # Bias
            knob_idx += 1
            hidden2_thoughts[h2] = math.tanh(score)

        # Calculate 9 Action Scores based on Layer 2
        action_scores = [0.0] * 9 
        knob_idx = 0
        for a in range(9):
            score = 0
            for h2 in range(6):
                score += hidden2_thoughts[h2] * self.action_knobs[knob_idx]
                knob_idx += 1
            score += self.action_knobs[knob_idx] # Bias
            knob_idx += 1
            action_scores[a] = score

        return action_scores, hidden1_thoughts, hidden2_thoughts, inputs

    def save_to_file(self, filename="best_brain_v4.json"):
        data = {
            "hidden1_knobs": self.hidden1_knobs, 
            "hidden2_knobs": self.hidden2_knobs, 
            "action_knobs": self.action_knobs
        }
        with open(filename, "w") as f:
            json.dump(data, f, indent=4)

    def load_from_file(self, filename="best_brain_v4.json"):
        with open(filename, "r") as f:
            data = json.load(f)
            self.hidden1_knobs = data["hidden1_knobs"]
            self.hidden2_knobs = data["hidden2_knobs"]
            self.action_knobs = data["action_knobs"]

# ================= 2. THE VISUALIZER =================
def brain_visualizer_thread():
    root = tk.Tk()
    root.title("Deep Neural Network Activity")
    ...
    def update_ui():
        ...
        layers = [
            ...
            {"count": 6,  "x": 150, "data": ui_data["hidden1"], "name": "Layer 1"},
            {"count": 6,  "x": 250, "data": ui_data["hidden2"], "name": "Layer 2"},
        ...
        for layer in layers:
            ...
            for i in range(layer["count"]):
                ...
                if layer["name"] == "Output" and i == 8:
                    ...
                    color = "#ff0000" if is_braking else "#333333" 
...
def the_mind_thread():
    ...
    # Switched to v4 file
    if os.path.exists("best_brain_v4.json"):
        veteran = DeepRobotBrain()
        veteran.load_from_file("best_brain_v4.json")
    ...
    while True:
        ...
        for brain in population:
            ...
            for step in range(1200): 
                ...
                scores, _, _, _ = brain.decide_action(rel_angle, dist, r_yaw)
        ...
        if best_score_this_gen < 0.9: 
            target_dist = min(20.0, target_dist + 0.5) 
            if target_dist > 4.0: brain_ready = True

        best_global_brain = copy.deepcopy(best_brain_this_gen)
        best_global_brain.save_to_file("best_brain_v4.json") # Save as v4
...
while True:
    ...
    if dist < 1.0:
        ...
        p.resetBasePositionAndOrientation(duck, [random.uniform(-10,10), random.uniform(-10,10), 0.5], [0,0,0,1], body_client)
        p.resetBasePositionAndOrientation(robot, [0,0,0.5], [0,0,0,1], body_client)
        time.sleep(0.5)
        continue
    ...
    # Get inputs, outputs, AND both hidden layers
    scores, hid1, hid2, ins = current_brain.decide_action(rel_angle, dist, r_yaw)
    
    # Update UI Dictionary
    ui_data["inputs"] = ins
    ui_data["hidden1"] = hid1
    ui_data["hidden2"] = hid2
    ui_data["outputs"] = scores
    ...
    for i in range(4):
        ...
        p.setJointMotorControl2(robot, wheel_joints[i], p.VELOCITY_CONTROL, targetVelocity=target_vel, force=50, physicsClientId=body_client)
